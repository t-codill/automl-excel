...
interface IRunAMLSettings {
  primary_metric: string;
  data_script: string | null;
  compute_target: string;
  n_cross_validations: number;
  max_cores_per_iteration: number;
  blacklist_algos: never[];
  exit_score: number;
  name: string;
  task_type: string;
}

export const parentSuccessRunAMLSettings: IRunAMLSettings = {
  primary_metric: "AUC_weighted",
  data_script: null,
  compute_target: "local",
  n_cross_validations: 3,
  max_cores_per_iteration: 1,
  blacklist_algos: [] as never[],
  exit_score: 100,
  name: "my_experiment",
  task_type: "classification"
};

export const parentSuccessRunAMLSettingsWithMaximumCore: IRunAMLSettings = { ...parentSuccessRunAMLSettings, max_cores_per_iteration: -1 };

function getparentSuccessRun(amlSettings: IRunAMLSettings): NotNullableProperties<IParentRunData> {
  return {
    run: {
      runId: "AutoML_000",
...
export const parentSuccessRun = getparentSuccessRun(parentSuccessRunAMLSettings);
export const parentSuccessRunWithMaximumCore = getparentSuccessRun(parentSuccessRunAMLSettingsWithMaximumCore);
